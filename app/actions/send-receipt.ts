...
export async function sendReceiptEmail(email: string, order: any) {
    try {
        ...
        // Attempt to find card details from tenders
        const tender = order.tenders?.[0];
        const cardBrand = tender?.cardDetails?.card?.cardBrand || "Card";
        const last4 = tender?.cardDetails?.card?.last4 || "****";

        // 2. Build the Custom HTML Receipt
        // You can edit this HTML to look exactly how you want!
        const emailHtml = `
    ...
    `;
        ...
        const data = await resend.emails.send({
            from: 'Down South Corals <onboarding@resend.dev>', // You can customize this later with a real domain
            to: [email],
